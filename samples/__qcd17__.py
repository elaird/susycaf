...
qcd17.add("qcd_mg_ht_100_250",  '%s/%s/%s")'%(srm,mgQcdLoc,mgQcdDset%"100To250"), xs = {"LO":7e+06, "fakeNLO":7e+06*mgKFactor}["fakeNLO"])
qcd17.add("qcd_mg_ht_250_500",  '%s/%s/%s")'%(srm,mgQcdLoc,mgQcdDset%"250To500"), xs = {"LO":171e+03, "fakeNLO":171e+03*mgKFactor}["fakeNLO"])
qcd17.add("qcd_mg_ht_500_1000", '%s/%s/%s")'%(srm,mgQcdLoc,mgQcdDset%"500To1000"),xs = {"LO":5200, "fakeNLO":5200*mgKFactor}["fakeNLO"])
qcd17.add("qcd_mg_ht_1000_inf", '%s/%s/%s")'%(srm,mgQcdLoc,mgQcdDset%"1000"),     xs = {"LO":83, "fakeNLO":83*mgKFactor}["fakeNLO"])
...
for low,high,xs in zip(bins[:-1],bins[1:],xss) :
    qcd17.add("qcd_py6_pt_%s"%("%d_%d"%(low,high) if high else "%d"%low),
...
for low,high,xs in zip(bins[:-1],bins[1:],xss[:-1]) :
    qcd17.add("qcd_py6fj_pt_%s"%("%d_%d"%(low,high) if high else str(low)),
...
for low,high,xs,form in zip(bins[:-1],bins[1:],xss[:-1],forms[:-1]) :
    qcd17.add("qcd_py6fjmu_pt_%s"%("%d_%d"%(low,high) if high else str(low)),
...
for low,high,xs,_ in zip(bins[:-1],bins[1:],xss[:-1],forms[:-1]) :
    qcd17.add("qcd_mu_%s"%("%d_%d"%(low,high) if high else str(low)),
...
qcd17.add("qcd_mg_ht_100_250_summer11",  '%s/%s/%s")'%(srm, qcdDir1, qcdDset1%"100To250"), xs = {"LO":4194000.0, "fakeNLO":4194000.0*mgKFactor}["fakeNLO"])
qcd17.add("qcd_mg_ht_250_500_summer11",  '%s/%s")'   %(srm, qcdDir2,                    ), xs = {"LO": 198500.0, "fakeNLO": 198500.0*mgKFactor}["fakeNLO"])
qcd17.add("qcd_mg_ht_500_1000_summer11", '%s/%s/%s")'%(srm, qcdDir1, qcdDset1%"500To1000"),xs = {"LO":   5856.0, "fakeNLO":   5856.0*mgKFactor}["fakeNLO"])
qcd17.add("qcd_mg_ht_1000_inf_summer11", '%s/%s/%s")'%(srm, qcdDir1, qcdDset1%"1000"),     xs = {"LO":    122.6, "fakeNLO":    122.6*mgKFactor}["fakeNLO"])

# QCD (HT binned) skims
dir = "/vols/cms02/elaird1/29_skims/04_photons/v8"
l = 'utils.fileListFromDisk(isDirectory = False, location = '
qcd17.add("qcd_mg_ht_100_250_summer11_skim",   '%s"%s/qcd_mg_ht_100_250_summer11_*_skim.root")'%(l, dir), xs = 3.180463e-06 * 5.326380e+06)
qcd17.add("qcd_mg_ht_250_500_summer11_skim",   '%s"%s/qcd_mg_ht_250_500_summer11_*_skim.root")'%(l, dir), xs = 2.491150e-04 * 2.520950e+05)
qcd17.add("qcd_mg_ht_500_1000_summer11_skim", '%s"%s/qcd_mg_ht_500_1000_summer11_*_skim.root")'%(l, dir), xs = 2.405978e-04 * 7.437120e+03)
qcd17.add("qcd_mg_ht_1000_inf_summer11_skim", '%s"%s/qcd_mg_ht_1000_inf_summer11_*_skim.root")'%(l, dir), xs = 1.675973e-04 * 1.557020e+02)
