...
class cleanJetHtMhtHistogrammer(analysisStep) :
    ...
    def uponAcceptance (self,eventVars) :
        ...
        self.book.fill(       mht+ht,"%sHtPlusMhtRough%s"%self.cs, 15,  500.0, 2000.0, title = ";H_{T} + #slash{H}_{T} (GeV) from %s%s %s_{T}'s;events / bin"%(self.cs[0],self.cs[1],self.letter))        
...
class alphaHistogrammer(analysisStep) :
    ...
    def uponAcceptance (self,eventVars) :
        ...
        deltaPhiStar = eventVars[self.DeltaPhiStar][0][0]
